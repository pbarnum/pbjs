...
(function() {
  ...
  pbjs.trees = pbjs.trees.createTree = function(id, options) {
    // Id must be present so we know where to build
    if (id && id !== String) {
      return pbjs.error("Tree View", "Missing element id in arguments list");
    }
    
    this.options = __setDefaults(options);
    
    this.root = null;
    this.nodes = [];
    this.elements = __getTreeElements();
    ...
    if (options.ecButtons.enable) {
      this.elements.ectoggle = __ecButtons();
    }

    // Node Tooltip
    if(options.tooltip.enable)
    {
      this.elements.tooltip = $('<div id="pbTree-tooltip"></div>').appendTo('body').hide();
    }

    // Node Hover
    if(options.hover.enable)
    {
      this.elements.highlighter = $('<div id="pbTree-hover"></div>').appendTo(nodes.root);
    }

    // Node Selected
    if(options.select.enable)
    {
      this.elements.selected = $('<div id="pbTree-selected"></div>').appendTo(nodes.root);
    }
    ...
    {
      this.elements.search = undefined;
    }
    ...
    this.root.find('a').each(function()
    {
      $(this).click(function()
      {
        ...
        if(options.select.enable)
        {
          this.elements.selected.show().css(_setSelTop());
        }
      });
    });
    ...
    this.root.click(function(e)
    {
      if((this.root.has(e.target).length > 0 && $(e.target).hasClass('pbTree-subRoot')) || e.target)
      {
        ...
        else
        {
          ...
          {
            this.elements.selected.css(_setSelTop()).show();
          }
        }
      }
    })
    ...
    {
      if(options.hover.enable
      && this.root.has(e.target).length > 0
      && ($(e.target).is('li') || $(e.target).is('a')))
      {
        this.elements.highlighter.show().css({
        ...
        if(options.tooltip.enable)
        {
          ...
          this.elements.tooltip.text(tex).css({
            top: e.clientY + $(window).scrollTop() - options.tooltip.offsetY - this.elements.tooltip.outerHeight() + 'px',
    ...
    .mouseleave(function()
    {
      ...
      {
        this.elements.highlighter.hide();
...
*/
  $.fn.pbTree.findOne = function(id, hide)
  {
    ...
    if($node.length > 0)
    {
      ...
      this.elements.selected.hide().css(_setSelTop()).show();
...
*/
  ...
  pbjs.trees.tree.prototype.showAllNodes() {
    ...
    if (this.options.select.enable) {
      this.elements.selected.setPosition();
    }
  };
  ...
  function __ecButtons() {
    var obj = {};
    obj.container = pbjs.trees.tree.elements.ectoggle;
    ...
    //var $collapseAll = this.elements.ectoggle.find('.pbTree-collapseAll')
    //.click(function() {
    //  _hideAll();
    //  nodes.root.animate({scrollTop: 0}, 10);
    //
    //  // Position the links back into frame
    //  this.elements.ectoggle.css('bottom', 0);
    //
    //  if (options.select.enable) {
    //    this.elements.selected.hide();
    //  }
