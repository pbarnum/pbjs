...
var $settings = {},
    // Plugin settings
searchArray = [],
    // Object holding all searchable objects
$container,
    // Element containing TreeView
$root,
    // Root list of the TreeView
$tooltip,
    // Tooltip div
$hoverDiv,
    // Hover div
...
var TreeView = function (options) {
    ...
    if ($settings.search.enable) {
        ...
        if ($settings.search.selector && typeof $settings.search.selector === 'string' && $($settings.search.selector).length > 0 && $($settings.search.selector).is('input:text')) {
            ...
            if (!$settings.search.selector) console.error('No search selector provided!');
            if (typeof $settings.search.selector !== 'string') console.error('Search selector must be a string!');
            if ($($settings.search.selector).length <= 0) console.error('The search element does not exist!');
            if (!$($settings.search.selector).is('input:text')) console.error('The search element must be a text field!');
    ...
    $root.find('a').each(function () {
        $(this).click(function () {
            ...
            if ($settings.showoccupied.enable) $(this).css($settings.showoccupied.style);
    ...
    $root.click(function (e) {
        if ($root.has(e.target).length > 0 && $(e.target).hasClass('pbTree-subRoot') || e.target) {
            ...
            if (eNode.hasClass('pbTree-opened')) {
                ...
                if ($settings.select.enable && eNode.find('a.pbTree-current').length > 0) $selDiv.hide();
            } else {
                _setState(eNode);

                if ($settings.select.enable && eNode.find('a.pbTree-current').length > 0 && eNode.find('li.pbTree-closed').length <= 0) {
    ...
    .mousemove(function (e) {
        if ($settings.hover.enable && $root.has(e.target).length > 0 && ($(e.target).is('li') || $(e.target).is('a'))) {
            $hoverDiv.show().css({
    ...
    $container.scroll(function () {
        ...
        if ($settings.select.enable && $('a.pbTree-current').length > 0 && $('a.pbTree-current').parents('li.pbTree-closed').length <= 0) $selDiv.css(_setSelTop()).show();
...
$.fn.pbTree.findOne = function (id, hide) {
    ...
    if ($node.length > 0) {
        ...
        if (!hide || typeof hide !== 'boolean') hide = true;

        // Hide all if applicable
        if (hide) _hideAll();

        if ($('a.pbTree-current').length > 0) $('a.pbTree-current').removeClass('pbTree-current');
...
function _ecButtons() {
    ...
    var $expandAll = $ecLinks.find('.pbTree-expandAll').click(function () {
        _showAll();

        if ($settings.select.enable && $('a.pbTree-current').length > 0) $selDiv.show().css(_setSelTop());

        return false;
    }).appendTo($ecLinks);

    var $collapseAll = $ecLinks.find('.pbTree-collapseAll').click(function () {
        _hideAll();
        ...
        if ($settings.select.enable) $selDiv.hide();
...
function _setState($this) {
    $this.each(function () {
        ...
        if ($(this).hasClass('pbTree-opened')) $(this).find('ul:first').show();else $(this).find('ul:first').hide();
    });
}
